...
def test_settings_reads_service_m2m_key_from_the_environment(monkeypatch):
    """
    Regression guard for the bug where apps/ai silently used the hardcoded
    default service_m2m_key instead of the real shared secret, because
    scripts/dev-local.ps1 never copied the root .env to apps/ai/.env
    (Settings reads `.env` relative to apps/ai's own working directory, per
    ...
    file" failure mode by itself. See scripts/local-e2e-smoke.mjs's
    waitForKnowledgeChunks() (end-to-end) and scripts/dev-local.ps1's
...
def test_default_core_base_url_matches_the_locked_local_api_port(monkeypatch):
    ...
    "http://127.0.0.1:3001", which does not match config/local-ports.json's
    ...
    `_env_file=None` isolates this from whatever apps/ai/.env happens to
