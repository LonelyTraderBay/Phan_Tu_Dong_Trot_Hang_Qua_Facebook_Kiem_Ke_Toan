...
**Staging hosts:** [r0-r3-execution-evidence.md](./r0-r3-execution-evidence.md#render-staging-services) · SDD Task 3 prep `2026-07-25` · **E3 Attempt R0.4** `2026-07-25`
...
|---|------|----------------|--------|
| 1 | Legal pages public (no auth) | Privacy + Terms URLs above | **UNVERIFIED** — E3 R0.4 local curl **timeout (28)** on `/legal/privacy` + `/legal/terms` (~15s); prior TLS reset. GHA keep-warm [30154127860](https://github.com/LonelyTraderBay/Phan_Tu_Dong_Trot_Hang_Qua_Facebook_Kiem_Ke_Toan/actions/runs/30154127860) `healthy_count=3/3` (web `/` HTTP 200) = host reachability when warm — **no direct `/legal/*` proof** |
| 2 | API always-on for webhook | `omni-api-staging` on Render Starter | **BLOCKED** — R0.2 still BLOCKED (free tier sleeps; keep-warm ≠ always-on) ([R0.2 owner upgrade](./deploy-staging-render.md#upgrade-to-always-on-owner)) |
...
| R0.4 gate | **BLOCKED** |
| Notes | Parent env: `META_APP_ID`/`META_APP_SECRET` len=7 placeholderish; `META_VERIFY_TOKEN` len=32 not placeholderish (local only). R0.2 always-on remains prerequisite for webhook reliability during review. Agent cannot submit App Review. |
