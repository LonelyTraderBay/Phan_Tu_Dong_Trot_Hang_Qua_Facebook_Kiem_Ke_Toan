...
|---|-----------|---------------|--------|
| 1 | Sign up + invite CSKH + kho | Signup → invite 2 roles → accept | **PASS** — Supabase signup + `POST /v1/orgs` + invites `cskh`/`kho` via API; invite accept link not exercised. `2026-07-25` · `sdd-task-1` |
...
| 3 | Create products; knowledge updates | Catalog CRUD → wait reindex / check chunks | **PASS** — `POST /v1/catalog/products` with variant OK; `knowledge_chunks` still 0 (reindex/outbox not verified this run). `2026-07-25` · `sdd-task-1` |
...
| 6 | Takeover pause/resume | Inbox → Chiếm quyền; bot_epoch; reply as staff | **PASS** — `POST /v1/inbox/conversations/:id/takeover` → `botPaused=true`, `botEpoch=1`; resume not exercised. `2026-07-25` · `sdd-task-1` |
...
**Overall R0.3:** **AMBER** — local §12.1 partially verified (4 PASS, 1 FAIL, 2 BLOCKED). Not claiming R0.3 GREEN until staging + Meta owner actions.
