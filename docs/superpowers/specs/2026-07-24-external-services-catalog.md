...
5. **Ngoại lệ gần như bắt buộc có phí nhỏ:** tên miền (domain) ~ vài trăm nghìn VND/năm. Có thể tạm dùng subdomain free (`*.pages.dev`) **chỉ khi chưa public thương hiệu**.
...
| Domain | Mua rẻ (Cloudflare Registrar / nhà VN) **hoặc** tạm `*.pages.dev` | Khi có khách: domain riêng bắt buộc (Meta App Review / brand) |
...
| Vercel Hobby ToS | Trung bình | Dùng **Cloudflare Pages** thay vì Vercel Hobby cho commercial |
...
### 3.1 Pre-customer (Free-first — đã chốt)

| Thành phần | Dịch vụ | Ghi chú |
|------------|---------|---------|
| `apps/web` | **Cloudflare Pages** Free | Next.js cần adapter (OpenNext / CF) — spike trong plan |
| `apps/api` | **Render Free** hoặc **Fly.io** free | Cold start / spin-down chấp nhận pre-customer |
| `apps/ai` | Cùng host với api (process riêng) | RAM ưu tiên hơn api |
| Workers | Process trên api/ai hoặc Inngest-managed | Không xử lý LLM trong HTTP webhook |

### 3.2 Khi có khách (upgrade)

| Thành phần | Hướng nâng |
|------------|------------|
| api + ai | Render/Fly **paid always-on** hoặc Cloud Run |
| web | Giữ CF Pages hoặc CF Workers paid nếu cần |
| DB | Supabase **Pro** (production) |

### 3.3 Phương án cloud “công ty lớn” (P2 nâng cấp sau)
...
### 3.3 Checklist thuê hosting P0

- [ ] Runtime **web** (production + staging)  
- [ ] Runtime **api** (production + staging)  
- [ ] Runtime **ai** (production + staging) — CPU/RAM đủ cho embed batch  
- [ ] HTTPS tự động  
- [ ] Log drain (xuất log ra dịch vụ quan sát — mục 8)  
- [ ] Autoscaling tối thiểu / max để kiểm soát bill LLM+AI  
...
Core enqueue, AI/Core workers consume.

| Lựa chọn | Loại | Ghi chú |
|----------|------|--------|
| **Inngest** | Managed jobs + backlog | Thuận Next/Nest; có free tier |
| **Trigger.dev** | Managed | Tương tự |
| **BullMQ + Redis** | Tự vận hành | Redis thuê **Upstash** / **Redis Cloud** / Railway Redis |
| **AWS SQS** + worker | Cloud native | Khi lên AWS |
| **Cloudflare Queues** | Opt | Nếu stack CF |

**P0:** một trong các dòng trên — **không** xử lý LLM/Meta nặng chỉ trong HTTP request.

**Nếu chọn BullMQ:** phải thuê thêm **Redis** (P0 kèm theo).
...
| **Preview env** | Cloudflare Pages preview + staging API | P1 |
...
| **Supabase Vault** | Encrypt token Meta | P0/P1 |
