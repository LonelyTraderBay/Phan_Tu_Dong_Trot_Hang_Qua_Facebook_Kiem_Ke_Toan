...
const REVOKE_MIGRATION = resolve(
  ROOT,
  'supabase/migrations/20260729000000_revoke_authenticated_writes_control_tables.sql',
);
...
async function assertSupabaseReachable() {
  try {
    ...
    throw new Error(
      `Local Supabase unreachable at ${SUPABASE_URL} (${message}). Start with \`supabase start\` (see tests/isolation/README.md).`,
    );
  }
}
